            ...
            @if(!empty($categoryItem->subCategories) and count($categoryItem->subCategories))
                <div class="accordion border-bottom-gray-100 mt-16 px-16 pb-16">
                    <div class="accordion__title d-flex align-items-center justify-content-between">
                        <div class="font-14 font-weight-bold text-dark cursor-pointer" href="#leftFiltersCategory{{ $categoryItem->id }}" data-parent="#leftFiltersAccordion" role="button" data-toggle="collapse">
                            {{ $categoryItem->title }}
                        </div>

                        <span class="collapse-arrow-icon d-flex cursor-pointer" href="#leftFiltersCategory{{ $categoryItem->id }}" data-parent="#leftFiltersAccordion" role="button" data-toggle="collapse">
                            <x-iconsax-lin-arrow-up-1 class="icons text-gray-500" width="16"/>
                        </span>
                    </div>

                    <div id="leftFiltersCategory{{ $categoryItem->id }}" class="accordion__collapse pt-0 mt-0 border-0 {{ $loop->first ? 'show' : '' }}" role="tabpanel">
                        <div class="pl-8">
                            @foreach($categoryItem->subCategories as $subCategory)
                                <div class="js-get-view-data-by-tab mt-16 cursor-pointer" data-filter-name="category_id" data-filter-value="{{ $subCategory->id }}" data-container-id="listsContainer">{{ $subCategory->title }}</div>
                            @endforeach
                        </div>
                    </div>
                </div>
            @else
                <div class="js-get-view-data-by-tab mt-16 px-16 cursor-pointer font-weight-bold" data-filter-name="category_id" data-filter-value="{{ $categoryItem->id }}" data-container-id="listsContainer">{{ $categoryItem->title }}</div>
            @endif
        @endforeach
    </div>
</div>
